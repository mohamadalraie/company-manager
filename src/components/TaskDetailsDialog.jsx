...
import EditTaskDialog from './EditTaskDialog'; // **استيراد ديالوج التعديل**
...
const TaskDetailDialog = ({ open, onClose, task: initialTask, onTaskDeleted, onTaskUpdated, consultingCompanyId, participants,stageId }) => {
  ...
  // --- Local State for the task to allow live updates ---
  ...
  // --- Action handlers ---
  const handleEdit = () => {
    setIsEditDialogOpen(true); // **فتح ديالوج التعديل**
    handleMenuClose();
  };
  ...
  const handleTaskUpdateSuccess = (updatedTaskData) => {
    // Update the local task state to reflect changes immediately
    const newTaskState = { ...task, ...updatedTaskData.data };
    setTask(newTaskState);
    
    // Notify the parent component (e.g., the main task board)
    if (onTaskUpdated) {
      onTaskUpdated(newTaskState);
    }
  ...
  return (
    <>
      <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg" PaperProps={{ sx: { backgroundColor: colors.primary[800], borderRadius: '12px' } }}>
        ...
        <DialogContent sx={{ backgroundColor: colors.primary[800] }}>
          <Grid container spacing={4} sx={{ pt: 2 }}>
            <Grid item xs={12} md={8}>
              <Box>
                <Typography variant="h6" color={colors.grey[200]} gutterBottom>Description</Typography>
                <Typography variant="body1" sx={{ color: colors.grey[300], lineHeight: 1.7, mb: 4 }}>
                  {task.description}
                </Typography>
                <Divider sx={{ my: 2 }} />
                <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1.5 }}>
                  ...
                  <Box>
                    <Typography variant="h6" color={colors.grey[200]} gutterBottom>Notes</Typography>
            ...
            <Grid item xs={12} md={4} sx={{ borderLeft: { md: `1px solid ${colors.grey[700]}` }, pl: { md: "32px !important" } }}>
              <Box>
                ...
                <MetadataItem icon={<PersonPinIcon />} label="Assigned To">
                  <Box display="flex" alignItems="center" gap={1}>
                    <Avatar sx={{ width: 24, height: 24, fontSize: "0.8rem", bgcolor: colors.greenAccent[700] }}>
                ...
                <MetadataItem icon={<EventIcon />} label="Date Of Closing The Task">
                  {task.actual_date_of_closed === null ? "Not closed yet" : new Date(task.actual_date_of_closed).toLocaleDateString("en-GB")}
                </MetadataItem>
                <Divider sx={{ my: 2.5 }} />
        ...
        <DialogActions sx={{ p: 2, borderTop: `1px solid ${colors.grey[700]}`, backgroundColor: colors.primary[800] }}>
          <Button onClick={onClose} variant="contained" sx={{ color: colors.grey[100], backgroundColor: colors.greenAccent[700], '&:hover': { backgroundColor: colors.grey[700] } }}>
            Close
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={isDeleteDialogOpen} onClose={handleDeleteDialogClose} PaperProps={{ sx: { backgroundColor: colors.primary[800] } }}>
        ...
        <DialogActions>
          ...
          <Button onClick={handleConfirmDelete} color="error" disabled={isDeleting} startIcon={isDeleting ? <CircularProgress size={20} color="inherit"/> : null} variant="contained">
            {isDeleting ? "Deleting..." : "Delete"}
          </Button>
        </DialogActions>
      </Dialog>
      
      {/* --- **ديالوج التعديل الجديد** --- */}
      {task && (
        <EditTaskDialog
          open={isEditDialogOpen}
          onClose={() => setIsEditDialogOpen(false)}
          task={task}
          onTaskUpdated={handleTaskUpdateSuccess}
