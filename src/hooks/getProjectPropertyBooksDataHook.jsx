...
import { getProjectPropertyBookApi } from "../shared/APIs"; // افترض وجود هذا المسار
...
const useProjectPropertyBooksData = ({ projectId }) => {
  ...
  const refetchPropertyBooks = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(`${baseUrl}${getProjectPropertyBookApi}${projectId}`, config);
