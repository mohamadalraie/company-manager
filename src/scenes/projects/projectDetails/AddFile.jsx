...
const AddProjectFile = ({ open, onClose, projectId, onUploadSuccess }) => {
 ...
 try {
 const response = await axios.post(`${baseUrl}${addProjectFileApi(projectId)}`, formData, {
 headers: {
 ...
 showSnackbar(`Failed to upload file: ${response.data.message || 'Unknown error'}`, 'error');
 }
 } catch (error) {
 ...
 showSnackbar(`Project ID Error: ${validationErrors.project_id[0]}`, "error");
 }
 ...
 <InputLabel htmlFor="file-upload-button" sx={{ color: colors.greenAccent[300] }}>
 ...
 <OutlinedInput
        ...
        startAdornment={
            <InputAdornment position="start">
                ...
                <IconButton component="label" htmlFor="hidden-file-upload-button" sx={{ color: colors.blueAccent[300] }}>
                    <UploadFileIcon />
                </IconButton>
            </InputAdornment>
        }
        endAdornment={
            selectedFile && (
                <InputAdornment position="end">
                    <IconButton onClick={handleClearFile} edge="end" sx={{ color: colors.grey[400] }}>
                        <ClearIcon />
                    </IconButton>
                </InputAdornment>
            )
    ...
    />
    {fileError && <FormHelperText sx={{ color: colors.redAccent[500] }}>{fileError}</FormHelperText>}
 ...
 '&.Mui-disabled': { backgroundColor: colors.grey[600], color: colors.grey[400] },
 }}
 >
